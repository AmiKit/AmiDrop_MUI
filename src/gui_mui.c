...
/* Use the compiler toolchain's MUI proto/inline headers.  The external MUI
 * 3.8 SDK is added with -idirafter by the Makefile, so its old GCC-2.x
 * inline/muimaster.h cannot override the modern toolchain wrapper. */
...
struct AmiDropGui {
    ...
    /* About is a normal MUI subwindow, just like Preferences.  Keeping it
       inside the application tree makes it inherit the same WindowBack,
       button frames and user-configured MUI style instead of MUI_Request()'s
       separate requester background. */
    Object *about_window;
    Object *about_ok;
    ...
    /* Mirrors prefs->show_transfer_information for the life of this window.
       ...
       the interface, which is the one route its contract offers both
    ...
    return WindowObject,
        ...
        WindowContents, VGroup,
            ...
            Child, VGroup,
                ...
                Child, HGroup,
                    Child, HSpace(0),
                    Child, Label2((STRPTR)"No Code needed"),
                    Child, gui->prefs_no_code = MUI_MakeObject(MUIO_Checkmark, (ULONG)0),
                End,
...
static Object *build_about_window(struct AmiDropGui *gui)
{
    return WindowObject,
        MUIA_Window_Title     , (ULONG)"About AmiDrop",
        MUIA_Window_ID        , MAKE_ID('A','D','A','5'),
        MUIA_Window_Width     , 390,
        /* About is informational and has a fixed layout.  Preferences stays
           resizable, but this window deliberately has no size gadget. */
        MUIA_Window_SizeGadget, FALSE,

        WindowContents, VGroup,
            Child, VGroup,
                GroupFrameT((STRPTR)"About AmiDrop"),
            ...
            Child, HGroup,
                Child, HSpace(0),
                Child, gui->about_ok = SimpleButton("OK"),
                Child, HSpace(0),
            End,
...
static BOOL build_application(struct AmiDropGui *gui)
{
    ...
    /* Colons written out: MUIO_Label takes the text verbatim (mui.h has no
       flag that adds one), and the GadTools window shows "Address:", so this one was the odd one out.  ESC b turns on the bold
    ...
    gui->about_window = build_about_window(gui);
    ...
    if (!menustrip || !address_row || !code_row || !receive_row ||
        !status_row || !limit_row || !gui->qr_object ||
        !gui->prefs_window || !gui->about_window) {
        if (gui->about_window) MUI_DisposeObject(gui->about_window);
...
static void attach_notifications(struct AmiDropGui *gui)
{
    ...
    DoMethod(gui->about_window, MUIM_Notify, MUIA_Window_CloseRequest, TRUE,
             gui->app, 2, MUIM_Application_ReturnID, ABOUT_RID_CLOSE);
    DoMethod(gui->about_ok, MUIM_Notify, MUIA_Pressed, FALSE,
             gui->app, 2, MUIM_Application_ReturnID, ABOUT_RID_CLOSE);
...
{
    if (gui && gui->app) {
        /* Close all application windows first, then dispose the Application
           object once.  This is the canonical MUI shutdown sequence.  Do not
           rewrite CloseRequest during teardown: it is only an input-state
           attribute and disposal owns the object tree from here on. */
        if (gui->about_window) set(gui->about_window, MUIA_Window_Open, FALSE);
        if (gui->prefs_window) set(gui->prefs_window, MUIA_Window_Open, FALSE);
        if (gui->window) set(gui->window, MUIA_Window_Open, FALSE);
...
struct AmiDropGui *gui_create(const struct AmiDropPrefs *prefs)
{
    ...
    /* cl_ID naming of a private MUI custom class is a V20-era convenience.
       MUI 3.8 is V19 and this class is private, so no class ID is required
       there.  Keep the older runtime's teardown path as conservative as
       possible; newer MUI versions may still get the useful debug name. */
...
BOOL gui_next_event(struct AmiDropGui *gui, struct AmiDropGuiEvent *event)
{
    ...
    for (;;) {
        ...
        if (id == (ULONG)MUIV_Application_ReturnID_Quit) {
            /* The standard MUI close key (normally ESC) sets CloseRequest
               before the notification queues MUIV_Application_ReturnID_Quit.
               Reset it after consuming the request so a cancelled quit can
               be requested again later.  Teardown itself never touches this
               attribute. */
            if (gui->window)
                set(gui->window, MUIA_Window_CloseRequest, FALSE);
            event->type = GUI_EVENT_QUIT;
            return TRUE;
        }
        if (id == RID_ABORT) {
                ...
                const struct AmiDropPrefs *prefs)
{
    ...
    set_text(gui->code_text, gui->shown_code, sizeof(gui->shown_code),
             prefs->no_code_needed ? "Deactivated by user" :
       ...
       block. */
    {
        ...
        if (prefs->no_code_needed && server->address_valid) {
            strncpy(url, server->address, sizeof(url) - 1);
            url[sizeof(url) - 1] = '\0';
        } else {
...
void gui_show_about(struct AmiDropGui *gui)
{
    ...
    if (!gui || !gui->app || !gui->about_window) return;

    /* A real MUI subwindow intentionally replaces MUI_Request().  This makes
       About use exactly the same MUI window/button styling as Preferences,
       including the user's configured WindowBack and button frames. */
    set(gui->about_window, MUIA_Window_CloseRequest, FALSE);
    set(gui->about_window, MUIA_Window_Open, TRUE);
    get(gui->about_window, MUIA_Window_Open, &open);
    if (!open) return;

    /* main.c refuses About while a transfer is active, so pausing its input
       here cannot stall an upload.  Sleeping the main window also prevents
       its menu/buttons from adding return IDs to this application's one FIFO
       while the nested loop owns it. */
    set(gui->window, MUIA_Window_Sleep, TRUE);

    for (;;) {
        ULONG id = DoMethod(gui->app, MUIM_Application_NewInput, &signals);

        if (id == ABOUT_RID_CLOSE) {
            set(gui->about_window, MUIA_Window_CloseRequest, FALSE);
            break;
        }
        if (id == (ULONG)MUIV_Application_ReturnID_Quit) {
    ...
    set(gui->about_window, MUIA_Window_CloseRequest, FALSE);
    set(gui->about_window, MUIA_Window_Open, FALSE);
    set(gui->window, MUIA_Window_Sleep, FALSE);

    /* NewInput above consumed the main loop's old signal state.  Re-prime it
       and drain stale dialog IDs before returning.  A quit is preserved out
       of band because a later layout rebuild may dispose this application. */
    {
        ULONG fresh = 0;
        ULONG leftover;
...
int gui_preferences(struct AmiDropGui *gui, struct AmiDropPrefs *prefs)
{
    ...
    SetAttrs(gui->prefs_no_code, MUIA_Selected, prefs->no_code_needed ? TRUE : FALSE, TAG_DONE);
    ...
    /* ESC uses MUIA_Window_CloseRequest for this subwindow too.  Clear any
       previous request before reopening it so the next Preferences session
       starts from the same clean state as About and the main window. */
    set(gui->prefs_window, MUIA_Window_CloseRequest, FALSE);
    ...
    for (;;) {
        ...
        if (id == PREFS_RID_SAVE || id == PREFS_RID_USE) {
            ...
            get(gui->prefs_no_code, MUIA_Selected, &value);
            prefs->no_code_needed = value ? TRUE : FALSE;
        ...
        if (id == PREFS_RID_CANCEL ||
            id == (ULONG)MUIV_Application_ReturnID_Quit) {
            if (requester_is_open(gui)) { signals = 0; continue; }
