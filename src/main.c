...
/* Only the libraries every frontend needs are opened here.  Toolkit-specific
   ones (gadtools/asl or muimaster) belong to the frontend and are opened by
   gui_create(). */
...
static BOOL apply_named_option(struct AmiDropPrefs *prefs, const char *name, const char *value)
{
    ...
    if (amidrop_ascii_casecmp_n(name, "NOCODENEEDED", 13) == 0 && name[12] == '\0') {
        if (!prefs_parse_yes_no(value, &boolean_value)) return FALSE;
        prefs->no_code_needed = boolean_value;
        return TRUE;
    }
...
{
    static const char *keys[] = { "PORT", "MAXSIZE", "MAXSIZEKB", "RECEIVEDIR", "STARTSERVER", "NOCODENEEDED", "IGNORESPACE", "SHOWTRANSFERINFO" };
    UWORD i;
    BOOL all_valid = TRUE;
...
/* The GadTools dialog asks this question inside itself.  The MUI dialog does
   not, so a limit the volume cannot hold would otherwise be accepted in
   silence.  Asking here covers both frontends with one
...
/* ShowTransferInformation changes the window layout, and the GadTools
   ...
   the one route the contract offers both frontends, and a path taken at
   most a few times in a session does not deserve one of its own. */
...
#define PREFS_APPLY_REJECTED 0
#define PREFS_APPLY_OK       1
#define PREFS_APPLY_FATAL   -1

static int apply_preferences(struct AmiDropGui **gui, struct AmiDropServer *server,
                             ...
                             const struct AmiDropPrefs *candidate, BOOL save)
{
    ...
    if (!gui || !*gui || !server || !prefs || !candidate) return PREFS_APPLY_REJECTED;

    /* confirm_capacity() may turn on ignore_free_space, so it needs a
       writable copy; everything below works from that copy. */
    accepted = *candidate;
    if (!confirm_capacity(*gui, &accepted)) return PREFS_APPLY_REJECTED;
    candidate = &accepted;
    ...
    if (layout_changed && !reopen_main_gui(gui, server, candidate)) {
        if (reopen_main_gui(gui, server, &old_prefs)) {
            ...
            return PREFS_APPLY_REJECTED;
        }
        gui_message(NULL, "AmiDrop fatal error",
                    "The main window could not be reopened. AmiDrop will exit so it can be started again safely.");
        return PREFS_APPLY_FATAL;
    }
    ...
    if (was_running && port_changed) {
        ...
        if (!server_start(server, candidate)) {
            ...
            if (layout_changed && !reopen_main_gui(gui, server, &old_prefs)) {
                gui_message(NULL, "AmiDrop fatal error",
                            "The previous main-window layout could not be restored. AmiDrop will exit so it can be started again safely.");
                return PREFS_APPLY_FATAL;
            }
            if (restored) {
                gui_message(*gui, "Preferences not applied",
                               ...
                               struct AmiDropPrefs *prefs, ULONG action)
{
    if (action == MID_QUIT) {
        if (server->uploading) {
            if (!gui_confirm(*gui, "Quit AmiDrop",
                             "A transfer is active. Quit AmiDrop and remove the incomplete file?",
                             "Quit", "Cancel")) {
                /* A modal requester pauses the receive loop.  Cancel means
                   keep the transfer, so do not let requester time count as
                   network-idle time and immediately delete the partial file. */
                server_reset_idle_timeout(server);
                return TRUE;
            }
        }
        return FALSE;
    ...
    if (action == MID_ABOUT) {
        if (server_is_uploading(server)) {
            /* About is modal in both frontends.  Do not stop recv()/timeout
               processing merely to display credits during a transfer. */
            copy_status(server, "About is unavailable while a file is being received");
        } else {
            gui_show_about(*gui);
        }
        ...
        if (!server->running) {
            ...
            if (gui_confirm(*gui, "Stop AmiDrop server",
                            "A transfer is active. Stop the server and remove the incomplete file?",
                            "Stop", "Cancel")) {
                server_stop(server);
                gui_force_qr_redraw(*gui);
            } else {
                /* Same modal-requester rule as Quit above. */
                server_reset_idle_timeout(server);
            }
        } else {
            server_stop(server);
        ...
        int prefs_action;
        if (server_is_uploading(server)) {
            /* A requester here would reproduce the same transfer-stall bug we
               are trying to avoid, so report it in the non-modal status line. */
            copy_status(server, "Preferences cannot be changed while a file is being received");
        } else {
            int apply_result;
            candidate = *prefs;
            prefs_action = gui_preferences(*gui, &candidate);
            if (prefs_action == PREFS_ACTION_USE || prefs_action == PREFS_ACTION_SAVE) {
                apply_result = apply_preferences(gui, server, prefs, &candidate,
                                                 prefs_action == PREFS_ACTION_SAVE);
                if (apply_result == PREFS_APPLY_FATAL) return FALSE;
            }
        }
    }
    return TRUE;
...
int main(int argc, char **argv)
{
    ...
    while (running) {
        ...
        /* A MUI close request has already entered the frontend at this point.
           Once it resolves to Quit, leave the loop immediately.  Do not touch
           gadgets, alerts or redraw state for one more iteration while the
           window is in its close path.  This also restores the fatal-GUI-exit
           rule used before the shared-frontend refactor. */
        if (!running) break;
