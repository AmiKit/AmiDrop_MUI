...
static const char version_string[] __attribute__((used)) =
    "$VER: AmiDrop_ReAction " AMIDROP_VERSION " (" AMIDROP_DATE ")";
...
int main(int argc, char **argv)
{
    ...
    if (!open_libraries()) {
        if (IntuitionBase) {
            gui_message(NULL, "AmiDrop startup error",
                        "AmiDrop ReAction requires AmigaOS 3.2+, ReAction, icon.library and bsdsocket.library.");
        } else {
            printf("AmiDrop ReAction: required AmigaOS libraries, ReAction or bsdsocket.library are missing.\n");
        }
        close_libraries();
        return 20;
    }
    ...
    if (!gui_open(&gui)) {
        gui_message(NULL, "AmiDrop startup error", "Could not open the ReAction window. Make sure the AmigaOS 3.2 ReAction classes are installed.");
