...
/* Inserts newlines at word boundaries so a message fits a narrow requester.
   Requesters do not word-wrap on any of the three frontends: Intuition clips,
...
/* Composes the link a sender needs: "<address>?t=<token>".  In the MUI
   ...
   two cannot say different things; the GadTools and ReAction frontends still
   compose their own copy, and those lines are upstream's.
